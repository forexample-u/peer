...
<head>
  ...
  <style>
    ...
    #header  { background: #1e1e1e; padding: 10px 15px; display: flex; justify-content: flex-end; }
    #messages { background: #121212; padding: 15px; display: flex; gap: 12px; overflow-y: auto; flex-direction: column; flex-grow: 1; }
    #uploadForm { background: #1e1e1e; padding: 15px; display: flex; gap: 10px; }
    #fileInput { padding: 10px; flex-grow: 1; border: 2px dashed#555; text-align: center; color: #aaa; cursor: pointer; width: 100%; }
    button { background: #667eea; padding: 10px 20px; border-radius: 20px; border: none; color: #fff; cursor: pointer; }
  ...
  <script>
    ...
    document.getElementById('uploadForm').onsubmit = async e => {
      ...
      for (let file of fileInput.files) {
        ...
        xhr.upload.onprogress = e => { if (e.lengthComputable) { button.textContent = 'Uploading... ' + Math.round((e.loaded / e.total) * 100) + '%'; } };
        xhr.onload = () => {
